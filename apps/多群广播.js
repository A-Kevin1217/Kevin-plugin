...
export class example extends plugin {
  ...
  async broadcastMessage(e) {
    ...
    if (e.group_id === sourceGroupId && (bloggers.includes(e.user_id) || admins.includes(e.user_id))) {
      ...
      const sendToGroup = async (groupId, retryCount = 0) => {
        try {
          const group = Bot[430860145].pickGroup(groupId);
  ...
  async toggleForwardOn(e) {
    ...
    for (const groupId of destinationGroupIds) {
      await Bot[430860145].pickGroup(groupId).muteAll(true);  // 开启全员禁言
    }
    for (const groupId of destinationGroupIds) {
      await Bot[430860145].pickGroup(groupId).sendMsg('狂欢结束~ 大家保持安静🤫');
    }
  }
  ...
  async toggleForwardOff(e) {
    ...
    for (const groupId of destinationGroupIds) {
      await Bot[430860145].pickGroup(groupId).sendMsg('❗欢呼吧！在接下来的时间里，聚集地的所有人都能无限制的发布且能看到你发的信息');
    }
  }
